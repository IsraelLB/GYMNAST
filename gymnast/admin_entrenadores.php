...
$query = 'SELECT id_Empleado, nombre_Empleado, apellido_Empleado, telefono_Empleado, correo_Empleado, direccion_Empleado, fecha_nacimiento_Empleado, contrasena_Empleado, titulacion, EXPERIENCIA,imagen FROM Tabla_entrenadores';
$where = [];
...
oci_execute($stid);
echo "<h2>Registrar nuevo ENTRENADOR</h2>\n";
echo "<form method='POST' action='funciones/insertar_entrenador.php'>\n";
...
echo "Titulación: <input type='text' name='titulacion' required>\n";
echo "Experiencia: <input type='text' name='experiencia' required>\n";
echo "Imagen: <input type='file' name='imagen'>\n";
echo "<input type='submit' value='Registrar'>\n";
echo "</form>\n";
echo "<br>\n";

echo "<h1>Tabla de entrenadores</h1>\n";
echo "<form method='GET' action='admin_entrenadores.php'>\n";
...
while (($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) != false) {
    ...
    foreach ($row as $item) {
        if ($count < 10) {
            ...
            oci_execute($stmt);
            echo "<td>" . $result . "</td>\n";
            echo "<td><form method='POST' action='funciones/eliminar_entrenador.php'><input type='hidden' name='id_empleado' value='" . $row['ID_EMPLEADO'] . "'/><input type='submit' value='Eliminar' style='background-color: red; color: white;'/></form></td>\n";
            if ($item !== null) {
                $imageData = $item->load();
                $image = base64_encode($imageData);
                echo "<td><img src='data:image/jpeg;base64,{$image}' width='100' height='100' /></td>\n";
            } else {
                echo "<td>No image</td>\n";
            }
            echo "</tr>\n";
...
?>
<h1>Actualizar</h1>
<form method="POST" action="funciones/actualizar_entrenador.php" enctype="multipart/form-data">
    <?php 
    ...
    <select id="campo" name="campo" required>
        ...
        <option value="apellido_empleado">Apellido del Empleado</option>
